...
select Village.name from city
join village
on City.CityID = village.CityID
where city.name = 'rajkot'

--2. Display city along with their villages & pin code.
select city.name as city, village.name as village, city.pincode as pincode 
...
select city.Name as City, count(village.CityID) as Count
from city join village 
on city.CityID = village.CityID
group by city.name
