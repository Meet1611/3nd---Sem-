...
SELECT REGION, SUM(SALES_AMOUNT) AS TOTAL_SALES_AMOUNT FROM SALES_DATA
GROUP BY REGION

--2. Display Average Sales Amount by Product
SELECT PRODUCT, AVG(SALES_AMOUNT) AS AVG_SALES_AMOUNT FROM SALES_DATA
GROUP BY PRODUCT

--3. Display Maximum Sales Amount by Year
SELECT YEAR, SUM(SALES_AMOUNT) AS TOTAL_SALES_AMOUNT FROM SALES_DATA
GROUP BY YEAR

--4. Display Minimum Sales Amount by Region and Year
SELECT REGION, YEAR, MIN(SALES_AMOUNT) AS MIN_SALES_AMOUNT FROM SALES_DATA
GROUP BY REGION, YEAR

--5. Count of Products Sold by Region
SELECT REGION, COUNT(PRODUCT) AS TOTAL_PRODUCT FROM SALES_DATA
GROUP BY REGION
...
SELECT YEAR, REGION, COUNT(PRODUCT) AS TOTAL_PROSUCT FROM SALES_DATA
...
SELECT YEAR, SUM(SALES_AMOUNT) AS TOTAL_AMOUNT FROM SALES_DATA
GROUP BY YEAR
...
SELECT PRODUCT, REGION, COUNT(PRODUCT) AS TOTAL_PRODUCT FROM SALES_DATA 
GROUP BY PRODUCT , REGION 
HAVING COUNT(PRODUCT) > 1

--5. Find out region wise highest sales amount.SELECT REGION, MAX(SALES_AMOUNT) AS HIGHEST_SALES FROM SALES_DATAGROUP BY REGION 